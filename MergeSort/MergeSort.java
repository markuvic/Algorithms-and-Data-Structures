...
public class MergeSort{
	...
	*/
	public static ListNode MergeSort(ListNode head){
            ...
            if (listLength(head)/2==0){return head;}
            ListNode slow = head;
            ListNode fast = head.next;
            
            while ( fast.next !=null && fast.next.next !=null ){//locate slow at half of the ListNode
                slow = slow.next;
                fast = fast.next.next;
            }
        
            ListNode half = MergeSort(slow.next);
            slow.next = null;
        
            return merge( MergeSort(head) , half );
	}
	/*public static ListNode getMiddle(ListNode slow,ListNode fast){
            if(fast.next==null){return slow;}
            ListNode mid = getMiddle(slow.next,fast.next.next);
            return mid;
        }*/
	public static ListNode merge(ListNode left,ListNode right){
        
            ListNode returnValue = new ListNode(-1);
            ListNode tmp = returnValue;
